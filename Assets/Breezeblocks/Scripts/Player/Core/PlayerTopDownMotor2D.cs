...
public class PlayerTopDownMotor2D : MonoBehaviour
{
    ...
    /// <summary>
    /// Applies a multiplicative runtime speed modifier on top of normal movement speed selection.
    /// </summary>
    ...
    private float GetTargetSpeed(int levelIndex)
    {
        if (hasExternalSpeedOverride)
            return externalSpeedOverride * externalSpeedMultiplier;

        float armorSpeedMultiplier = GetArmorMovementSpeedMultiplier();
        if (IsSprinting)
            return walkSpeedLevels[SpeedLevelsCount - 1] * sprintSpeedMultiplier * armorSpeedMultiplier * externalSpeedMultiplier;

        return walkSpeedLevels[levelIndex] * armorSpeedMultiplier * externalSpeedMultiplier;
    }
